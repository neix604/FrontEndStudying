...
for문 안에 onclick 이벤트를 넣어서 라디오 버튼을 누를 때마다 알림창이 팝업되게 하는 코드였다.<br/>
처음에는 onclick을 트리거로 쓸 목적이라 밖에다 둬야 for문이 돌아갈 줄 알았는데 그렇게 하니까 아예 실행 자체가 되지 않았다.<br/>
그래서 for문 안으로 onclick을 넣어봤더니 그제야 내가 원하는 실행결과가 나왔다.<br/>
하지만 무슨 원리인지 알 수가 없어서 스택오버플로우에 질문을 올렸더니 노드 리스트는 추상적인 개념이어서 onclick을 할 수 없어서 내가 짠 코드처럼 쓰는 게 맞다는 답변을 받았다.<br/>
틀리게 쓰지 않았다는 것도 알았고 노드 리스트에는 onclick이 안 먹힌다는 것도 알았는데 여전히 원리는 알 수가 없었다.<br/>
저녁 내내 고민하다가 개발자 친구에게 도움을 청하고 나서야 100% 이해할 수 있었다.<br/>
for문이 돌아가면서 이벤트 함수가 하나씩 만들어졌기 때문에 내가 의도한 대로 작동한 거였다.<br/>
완벽하게 이해하고 보니 정말 당연하고 쉬운 개념인데 그 전까지는 트리거라는 단어에 사로잡혀서 다른 생각을 할 수가 없었다.<br/>
좀 더 넓은 시야로 보고 다양한 가능성들을 염두에 뒀더라면 이렇게까지 오래 걸리지 않았을 텐데.<br/>
왜 이렇게 되는 걸까? 라는 생각을 하는 건 좋지만 너무 작은 단위로 쪼개서 하나에만 집착하는 건 좋지 못한 습관이다.<br/>
다음에 비슷한 상황을 맞았을 때는 더 유연하게 생각할 수 있을 것 같다.
